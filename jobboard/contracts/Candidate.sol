...
contract Oracle is Ownable {
    ...
    //"vera",5,"0xca35b7d915458ef540ade6068dfe2f44e8fa733c","0x692a70d2e424a56d2c6c27aa97d1a86395877b3a"
    ...
    // 0xef55bfac4228981e850936aaf042951f7b146e41, "123",1000, ["first","second","exam"],[0,0,10],[false,true,true]
    ...
    function give_me_a_chance(bytes32 _vac, bytes32 _cv) public onlyCandidate(_cv) {
        require (current_action[_vac][_cv].title != bytes32(0));
        cv_requests[_vac][_cv] = true;
    }

    function refresh_cv(bytes32 _vac, bytes32 _cv) public onlyEmployer(_vac) onlyRequested(_vac, _cv) {
        current_action[_vac][_cv] = vacancy_pipeline[_vac][0];
    }
...
contract Candidate is Withdrawable {
    ...
    function pause_cv(bytes32 _uuid) public onlyOwner {
        oracle.pause_cv(_uuid);
    }

    function unpause_cv(bytes32 _uuid) public onlyOwner {
        oracle.unpause_cv(_uuid);
    }

    function subscribe(bytes32 _vac_uuid, bytes32 _cv_uuid) public onlyOwner {
        oracle.subscribe(_vac_uuid, _cv_uuid);
    }
}
