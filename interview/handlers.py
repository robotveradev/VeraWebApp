...
from . import models
from interview import router
from interview.utils import get_user_from_session, get_dialogs_with_user, get_interview_by_id
...
def new_messages_handler(stream):
    while True:
        ...
        interview_id = packet.get('interview')
        if session_id and msg and interview_id:
            user_owner = get_user_from_session(session_id)
            if user_owner:
                dialog = get_interview_by_id(interview_id)
                if len(dialog) > 0:
                    # Save the message
                    ...
                    vac, cv = dialog[0].action_interview.action.pipeline.vacancy, dialog[0].cv

                    if cv.candidate.user == user_owner:
                        obj_1, obj_2 = cv, vac
                    else:
                        obj_1, obj_2 = vac, cv

                    if (obj_1.uuid, obj_2.uuid) in ws_connections:
                        connections.append(ws_connections[(obj_1.uuid, obj_2.uuid)])

                    # Find socket of the opponent
                    if (obj_2.uuid, obj_1.uuid) in ws_connections:
                        connections.append(ws_connections[(obj_2.uuid, obj_1.uuid)])
                    else:
                        # Find sockets of people who the opponent is talking with
                        opponent_connections = list(filter(lambda x: x[0] == obj_2.uuid, ws_connections))
...
def main_handler(websocket, path):
    ...
    path = path.split('/')
    user = get_user_from_session(path[2])
    interview = get_interview_by_id(path[3])[0]
    if interview:
        # Persist users connection, associate user w/a unique ID
        if interview.cv.candidate.user == user:
            ws_connections[(interview.cv.uuid, interview.action_interview.action.pipeline.vacancy.uuid)] = websocket
        else:
            ws_connections[(interview.action_interview.action.pipeline.vacancy.uuid, interview.cv.uuid)] = websocket
        ...
        try:
            ...
            del ws_connections[(interview[0].action_interview.action.pipeline.vacancy.uuid, interview[0].cv.uuid)]
    else:
        logger.info("Got invalid interview_id attempt to connect " + path[2])
